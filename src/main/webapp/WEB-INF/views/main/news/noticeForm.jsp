<%@ page language="java" contentType="text/html; charset=UTF-8"
    pageEncoding="UTF-8"%>
<div id="mainBanner" style="background-size:cover;background-image:url('<%=ctx%>/img/top-banner.jpg')" class="mb-4  w-full ">
	<div id="bannerText">
		<h1 class="border-l-4 border-brand-600 text-5xl text-white roboto-slab font-bold">&nbsp;&nbsp;NOTICE</h1>
	</div>
</div>
<div id="profileMenu" class="my-0 mx-auto w-full max-w-screen-xl">공지사항</span>
	<div id="hr" class="my-4 border-t border-solid"></div>
	<span class="font-bold text-xl">EduCamp 공지사항 테스트중 </span><span class="text-lg float-right">2020-06-14</span>
	<div class="border-t  border-b  mt-4 border-solid border-gray-500 h-64 mb-8">
		<div class="m-0 m-auto mt-4 w-11/12 text-black">EduCamp 공지사항 테스트 중</div>
	</div>
</div>
